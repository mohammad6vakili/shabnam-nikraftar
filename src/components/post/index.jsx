...
const Post = ({index, post}) => {
    ...
    return (
        <div key={index} className="post">
            ...
            >
                {!loading && post && (
                    post.attributes && post.attributes.image && post.attributes.image.length ?
                        <img
                            onClick={() => dispatch(setPostPreview({url: baseImageUrl + post.attributes.image[0].url}))}
                            src={baseImageUrl + post.attributes.image[0].url}
                            alt="post"
                        />
                        :
                        <img
                            onClick={() => dispatch(setPostPreview({url: postImage}))}
                            src={postImage}
                            alt="post"
                        />
                ...
                <div
                    onClick={() =>
                        dispatch(setPostPreview({
                            url: post.attributes && post.attributes.image && post.attributes.image.length ?
                                baseImageUrl + post.attributes.image[0].url :
                                postImage
                        }))}
